...
import { View, Text, StyleSheet, TouchableOpacity, Image, Modal, ScrollView,TextInput,ActivityIndicator } from 'react-native';
 ...
 const [isTrue,setISTrue] = useState(false);
  ...
  const getLocation= async ()=>{
    let currentLocation = await Location.getCurrentPositionAsync({});
      ...
      await fetch(`http://${ip}:3000/receive-location`, {
        method: 'POST',
        headers: {
      ...
      .then(data => {
        ...
        if (data) {
          const { city, dataList } = extractData(data);
  ...
  const extractData = (serverResponse) => {
    const { city, plan } = serverResponse;
    const dataList = [];
  
    // Extract data for each activity
    for (const [time, activity] of Object.entries(plan)) {
      const { cost, description, titleActivity } = activity;
      dataList.push({
        time,
        cost,
        description,
  ...
  const handlePress = async () => {
    ...
    await axios.post(`http://${ip}:3000/data`, Data)
      .then(response => {
        getLocation();
  ...
  return (
    ...
    />
      ...
      <TouchableOpacity style={[styles.resetButton, isFetching && styles.resetButtonDisabled]} onPress={handlePress} disabled={isFetching}>
